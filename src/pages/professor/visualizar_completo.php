...
try {
    ...
    $query =
        "SELECT
            ...
            ON
                p.CodigoSolicitacao = se.CodigoSolicitacao
            WHERE
    ...
    $query =
            "SELECT
                mi.Microscopia_id, mi.Microscopia, mi.Diagnostico, mi.Nota, mi.Patologista, se.StatusSolicitacao
            FROM
                Microscopia AS mi
            JOIN
                Paciente AS p
            ON
                mi.Paciente_id = p.Paciente_id
            JOIN
                SolicitacaoExame AS se
            ON
                p.CodigoSolicitacao = se.CodigoSolicitacao
            WHERE
                mi.Paciente_id = :Paciente_id";
    $stmt = $pdo->prepare($query);
    $stmt->bindParam(':Paciente_id', $paciente_id, PDO::PARAM_INT);
    $stmt->execute();
