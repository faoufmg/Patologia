...
<body>
    ...
    <div class="galeria">
        <?php
        try {
            $query = "SELECT * FROM FotoClinica WHERE DadosLesao_id = :DadosLesao_id";
            ...
            if ($arquivos) {
                foreach ($arquivos as $arquivo) {
                    $id_unico_arquivo = $arquivo['FotoClinica_id'];
                    $tipo_arquivo = $arquivo['TipoFoto']; // Ex: 'image/jpeg' ou 'application/pdf'
                    $caminho_arquivo = $arquivo['CaminhoFoto'];

                    // echo $caminho_arquivo;

                    // Define o endpoint que servirá o arquivo
                    $url_arquivo = 'exibir_foto.php?id=' . htmlspecialchars($id_unico_arquivo);
                    ...
                    if (strpos($tipo_arquivo, 'image/') === 0) {
                        echo '<img src="' . $url_arquivo . '" alt="Imagem Clínica">';
                ...
                echo "<p>Nenhuma imagem encontrado para esta lesão.</p>";
            }
