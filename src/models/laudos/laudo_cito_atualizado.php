...
try {
    $query =
            ...
            LEFT JOIN Laudos AS La ON P.Paciente_id = La.Paciente_id
            LEFT JOIN Assinaturas AS A ON A.Assinaturas_id = La.Assinaturas_id
            LEFT JOIN SolicitacaoCadastro AS SC ON SC.SolicitacaoCadastro_id = A.SolicitacaoCadastro_id
    ...
    $assinatura_base64 = $resultado && $resultado['Assinatura']
        ? base64_encode($resultado['Assinatura'])
        : '';
    $cro = $resultado['CRO'] ?? '';
    $nomeProfessor = htmlspecialchars($resultado['Usuario']);
    $dataHoraLaudo = date('d/m/Y H:i:s', strtotime($resultado['DataLiberacao']));
    ...
    <html>
        <head>
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .signature-block p { font-size: 14px; margin-bottom: 5px; margin-top: -10px; }
        ...
        <body>
            ...
            <h2 class="report-title">RELATÓRIO CITOPATOLÓGICO</h2>
